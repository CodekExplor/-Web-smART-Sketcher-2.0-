...
4. W trybie **Obraz** wybierz lub przeciągnij PNG, JPG, WEBP albo BMP. Możesz też wkleić bezpośredni adres HTTPS obrazu i kliknąć **Wczytaj z URL**. Serwer obrazu musi zezwalać na pobieranie z innych witryn (CORS); link do strony z obrazem nie wystarczy.
5. W trybie **Napis** wpisz tekst (również wielowierszowy). Kliknij **Pokaż czcionki z komputera** i zezwól przeglądarce na dostęp albo wskaż plik TTF, OTF, WOFF lub WOFF2. Wybierz rozmiar maksymalny, odstęp wierszy, wyrównanie i kolory. Aplikacja zawija tekst i zmniejsza go, gdy nie mieści się w kadrze; bardzo długi napis trzeba skrócić.
6. Dla obrazu wybierz **Dopasuj** (cały obraz i białe marginesy) lub **Wypełnij** (środkowe przycięcie). Ustaw skalę od 50% do 300% i obracaj obraz przyciskami o 90° w lewo lub w prawo. Opcja **Zachowaj cienkie linie** pomaga przy zmniejszaniu rysunków konturowych; wyłącz ją dla zdjęć.
7. Przesuń obraz lub napis suwakami X/Y, przeciągając podgląd albo strzałkami; Shift zwiększa krok do 5 px. Suwaki **Grubość linii**, **Kontrast** i **Jasność** pozwalają dopasować wygląd projekcji.
8. Sprawdź końcowy podgląd i kliknij **Wyślij do projektora**. W trybie obrazu przycisk **Resetuj ustawienia** przywraca skalę, obrót, położenie, grubość, kontrast i jasność do wartości początkowych.

Obraz i napis są przetwarzane lokalnie w przeglądarce. Przy użyciu URL przeglądarka pobiera obraz bezpośrednio z podanego serwera; aplikacja nie przesyła go ani wybranej czcionki na własny serwer. Podgląd pokazuje dokładnie obraz 160 × 128 RGB565 wysyłany do projektora. Skala obrazu działa względem wybranego trybu Dopasuj/Wypełnij; przy zmniejszeniu mogą pojawić się białe marginesy. PWA można zainstalować z menu Chrome/Edge; działa nadal w przeglądarkowym runtime i wymaga dostępnego Bluetooth. Lista czcionek zainstalowanych na komputerze wymaga obsługi Local Font Access API i zgody użytkownika; wybór pliku czcionki działa niezależnie od tej listy.
...
1. Otwórz [adres z numerem wersji](https://codekexplor.github.io/-Web-smART-Sketcher-2.0-/?v=20260924-8). Na dole strony musi być napis **Wersja 2026.09.24.8**. Zamknij wcześniej otwartą kartę lub zainstalowaną PWA. Zasoby JS/CSS mają numer wersji w adresie, a service worker pobiera aktualną wersję z sieci.
